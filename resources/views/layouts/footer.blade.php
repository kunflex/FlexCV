<!-- ======Footer======= -->
<div class="footer-layer">
    <h2>Build a compelling CV to showcase your skills and experience.</h2>
    <button>Explore More</button>
</div>
<div class="footer-container">
            <footer>
                <nav>
                    <ul>
                        <div class="conpany-logo">
                            <div class="logo-design" style="margin-left:-25px;">
                                <a href="{{route('landingpage')}}"><h1>Flex<span>CV</span></h1></a>
                             </div>
                            
                             <div style="width:250px;">
                                <p>
                                    P.O.Box 212 <br>
                                    Accra - Ghana <br><br>
                                   <b> Phone:</b> +(233)593958236 <br>
                                    <b>Email:</b> flexcv_info@gmail.com
                                </p>
                            </div>
                            <div></div>
                     
                        </div>
                        
                        <div class="footer-content">
                                <div style="width:40%;">
                                    <h3>About</h3><br>
                                    Welcome to our FlexCV, we empower individuals to 
                                    create compelling resumes that highlight their unique 
                                    strengths and experiences, ensuring their CV stands 
                                    out in the job market.
                                </div>
                                
                                <div>
                                    <h3>Features</h3><br>
                                    <ul>
                                        <li><b>&#10095; </b><a href="">Resume</a></li>
                                        <li><b>&#10095; </b><a href="">Templates</a></li>
                                        <li><b>&#10095; </b><a href="">Jobs</a></li>
                                    </ul>
                                </div>
                                <div>
                                    <h3>Support</h3><br>
                                    <ul>
                                        <li><b>&#10095; </b><a href="">Contacts</a></li>
                                        <li><b>&#10095; </b><a href="">Help Center</a></li>
                                        <li><b>&#10095; </b><a href="">FAQs</a></li>
                                    </ul>
                                </div>
                        </div>
                    </ul>
                </nav>
            </footer>
            
            <div class="handles">
                <div class="social-media-channels">
                    <svg fill="black" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="-143 145 512 512" xml:space="preserve">
                        <path d="M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M43.1,518.7
                        c-6.2,6.2-14.7,9.9-24.1,9.9c-9.4,0-17.8-3.8-24-9.9c-6.2-6.2-10-14.6-10-23.9c0-9.4,3.8-17.8,10-24s14.6-10,24-10
                        c9.4,0,17.9,3.8,24,10c6.2,6.2,10,14.6,10,24C53,504.2,49.2,512.6,43.1,518.7z M104.8,529c-0.1-32.1-12.5-62.3-35.1-84.9
                        c-22.6-22.6-52.8-35.2-84.7-35.2V360c46.6,0,88.7,19,119.3,49.6c30.6,30.6,49.5,72.8,49.6,119.4H104.8z M192,529
                        c-0.1-114.2-92.8-207.1-206.9-207.1V273c70.6,0,134.5,28.7,180.8,75.1c46.3,46.4,75,110.3,75.1,180.9H192z"/>
                    </svg>

                    <svg fill="black" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="-143 145 512 512" xml:space="preserve">
                        <path d="M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M41.4,508.1H-8.5V348.4h49.9
                        V508.1z M15.1,328.4h-0.4c-18.1,0-29.8-12.2-29.8-27.7c0-15.8,12.1-27.7,30.5-27.7c18.4,0,29.7,11.9,30.1,27.7
                        C45.6,316.1,33.9,328.4,15.1,328.4z M241,508.1h-56.6v-82.6c0-21.6-8.8-36.4-28.3-36.4c-14.9,0-23.2,10-27,19.6
                        c-1.4,3.4-1.2,8.2-1.2,13.1v86.3H71.8c0,0,0.7-146.4,0-159.7h56.1v25.1c3.3-11,21.2-26.6,49.8-26.6c35.5,0,63.3,23,63.3,72.4V508.1z
                        "/>
                    </svg>

                    <svg fill="black" height="30px" width="30px" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="0 0 512 512"  xml:space="preserve">
                        <style type="text/css">
                        </style>
                        <g>
                        <path class="st0" d="M256,0C114.615,0,0,114.615,0,256s114.615,256,256,256s256-114.615,256-256S397.385,0,256,0z M260.926,217.391
                            c0.084,0.004,0.164,0.021,0.25,0.025c-0.875-3.752-1.328-7.664-1.328-11.682c0-28.307,22.951-51.258,51.262-51.258
                            c14.742,0,28.063,6.225,37.414,16.188c9.6-1.89,18.684-5.174,27.129-9.523c1.781-0.864,3.566-1.723,5.32-2.674
                            c-3.039,9.334-8.744,17.412-16.141,23.532c-2.004,1.576-4.062,3.098-6.326,4.344c0.154-0.018,0.304-0.052,0.457-0.071
                            c-0.15,0.093-0.275,0.22-0.428,0.312c8.402-1.005,16.459-3.051,24.111-5.942c1.715-0.592,3.428-1.191,5.127-1.852
                            c-6.842,10.159-15.453,19.095-25.375,26.259c0.098,2.197,0.148,4.406,0.148,6.631c0,67.736-51.558,145.842-145.844,145.842
                            c-28.947,0-55.891-8.484-78.576-23.028c4.01,0.473,8.092,0.715,12.229,0.715c19.136,0,37.014-5.269,52.383-14.34
                            c3.871-2.23,7.658-4.639,11.273-7.365c-0.098-0.002-0.187-0.026-0.285-0.028c0.094-0.073,0.196-0.136,0.289-0.209
                            c-19.422-0.358-36.184-11.539-44.574-27.747c-1.25-2.489-2.32-5.096-3.164-7.831c3.086,0.58,6.246,0.898,9.5,0.898
                            c3.391,0,6.666-0.436,9.871-1.063c1.197-0.168,2.406-0.308,3.586-0.502c-0.156-0.032-0.293-0.1-0.449-0.133
                            c0.162-0.042,0.336-0.056,0.496-0.1c-23.449-4.709-41.119-25.428-41.119-50.262c0-0.196,0.002-0.387,0.004-0.58l0.024-0.055
                            c5.521,3.064,11.693,5.092,18.23,5.94c1.609,0.269,3.221,0.516,4.832,0.657c-0.11-0.074-0.205-0.164-0.314-0.238
                            c0.152,0.006,0.297,0.036,0.447,0.041c-13.754-9.191-22.803-24.883-22.803-42.666c0-8.142,1.988-15.787,5.367-22.623
                            c0.539-1.028,1.018-2.078,1.637-3.074c22.711,27.822,55.516,46.971,92.801,52.15c4.16,0.605,8.332,1.144,12.553,1.388
                            C260.934,217.443,260.932,217.416,260.926,217.391z"/>
                        </g>
                    </svg>

                    <svg fill="black" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                        viewBox="-143 145 512 512" xml:space="preserve">
                        <path d="M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M169.5,357.6l-2.9,38.3h-39.3
                        v133H77.7v-133H51.2v-38.3h26.5v-25.7c0-11.3,0.3-28.8,8.5-39.7c8.7-11.5,20.6-19.3,41.1-19.3c33.4,0,47.4,4.8,47.4,4.8l-6.6,39.2
                        c0,0-11-3.2-21.3-3.2c-10.3,0-19.5,3.7-19.5,14v29.9H169.5z"/>
                    </svg>

                </div>
                    <div class="agreement-cont">
                        <div class="agreement"><span>Terms & Conditions | Privacy Policies | Accessibility</span></div>
                    </div>
               </div>
                @php
                    $currentYear = now()->format('Y');
                @endphp
        <div class="copyright">&copy; {{ $currentYear }} <b>FlexCV Limited</b>. All Copyright Reserved | Theme powered by <b style="color:red">&#10084;</b> Kunflex <b>&trade;</b></div>
    </div>
    <!-- ======End Footer======= -->
